...
def scheme_read(src):
    """Read the next complete expression from SRC, a Buffer of tokens.
    but the return will not neccessary be a valid scheme list(i.e. Pair instance)
    ...
    if src.current() is None:
        raise EOFError
    val = src.pop_first() # Get and remove the first token,so the src has moved forward
    if val == 'nil':
        # BEGIN PROBLEM 1
        "*** YOUR CODE HERE ***"
        return nil
        ...
        # END PROBLEM 6
    elif val not in DELIMITERS:  #i.e. primitive expression
        return val
    else:
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"
            src.pop_first()   #no matter what the src.current() is,we always need to move forward the src
            return nil
            # END PROBLEM 1
        else:
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"
            var = scheme_read(src) # at the same time,src move forward
            rest = read_tail(src)
            return Pair(var,rest)
            # END PROBLEM 1
    except EOFError:
        raise SyntaxError('unexpected end of file')
