...
""" 我怀疑problem 6 中的一个测试有问题"""

from dice import six_sided, four_sided, eight_sided, make_test_dice, make_fair_dice
...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        ...
        for i in range(10, player_score + 1):
            if opponent_score % i == 0 and player_score % i ==0:
                return True
    return False
    # END PROBLEM 4a
...
def both(f, g):
    ...
    """
    def say(score0, score1):
        return both(f(score0, score1), g(score0, score1)) # 计算operand(f, g),返回echo_0,echo_1函数
    return say
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    """
    # 如何消除这个函数的side effect: 不进行变量重新赋值操作，而是直接将需要改变的变量传入函数中。
    # 比如不要进行nonlocal last_score, running_high   last_score = score1, running_high = this_increase操作
    # 上述操作会产生side effect.
...
# return max average score in 4 to 10 sided to roll 1 to 10 number dices. 
def max_average_score(trials_count = 50000):
	...
	for sided in range(4, 11):
		...
		# print("sided = {},roll_nums = {},max_average_score = {}".format(sided, average.index(max(average)), max(average)))
