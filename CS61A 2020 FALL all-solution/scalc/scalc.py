...
def calc_eval(exp):
    ...
    if type(exp) in (int, float):
        return simplify(exp) # 将能转为int的float转为int
    elif isinstance(exp, Pair):
        arguments = exp.second.map(calc_eval) # all but first依次调用calc_eval, 相当于计算subexpression
        return simplify(calc_apply(exp.first, arguments)) #寻找已命名的operator,进行计算
    else:
...
def calc_apply(operator, args):
    ...
    if operator == '+':
        return reduce(add, args, 0) #使用了reduce函数, 依次计算
    elif operator == '-':
        if len(args) == 0:
